...
public class TodoDAO extends DAO {
	...
	public int registerInsert(Todo dto) throws Exception {
		String sql = "INSERT INTO shohin (shohin_coode,shohin_lot,shohin_mei,shohin_bunrui,shouhin_quantity,serial_number,torokubi)VALUES (?,?,?,?,?,?,?)";
		...
		try {
			...
			PreparedStatement statement = getPreparedStatement(sql);
			statement.setString(1, dto.getShohin_coode());
			statement.setString(2, dto.getShohin_lot());
			statement.setString(3, dto.getShohin_mei());
			statement.setString(4, dto.getShohin_bunrui());
			statement.setInt(5, dto.getShouhin_quantity());
			statement.setInt(6, dto.getSerial_number());
			statement.setString(7, dto.getTorokubi());
			result = statement.executeUpdate();
	...
	public int registerUpdate(Todo dto) throws Exception {
		String sql = "UPDATE shohin SET  shohin_coode=?,shohin_lot=?,shohin_mei=?,shohin_bunrui=?,shouhin_quantity=?,serial_number=?,torokubi=? WHERE shouhin_id = ?";
		...
		try {
			...
			statement.setInt(6, dto.getSerial_number());
			statement.setString(7, dto.getTorokubi());
			statement.setInt(8, dto.getShouhin_id());
			result = statement.executeUpdate();
